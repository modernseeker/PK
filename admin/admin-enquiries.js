(()=>{
  ...
  function haystack(e){return `${ref(e)} ${e.customer_name} ${e.phone} ${e.business_name||''} ${e.location||''} ${e.notes||''} ${(e.enquiry_items||[]).map(i=>i.product_name).join(' ')}`.toLowerCase()}
  ...
  async function load(){const holder=$('#enquiryList');if(holder)holder.innerHTML='<div class="enquiry-loading">Loading enquiries…</div>';try{enquiries=await rest('enquiries?select=id,reference_code,customer_name,phone,business_name,location,notes,status,source,created_at,updated_at,enquiry_items(id,product_id,product_name,quantity)&order=created_at.desc');renderStats();renderList();if(selectedId)renderDetail(enquiries.find(e=>e.id===selectedId))}catch(e){if(holder)holder.innerHTML=`<div class="enquiry-loading error"><b>Could not load enquiries</b><span>${esc(e.message)}</span></div>`}}
