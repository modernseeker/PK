(()=>{
  ...
  function inject(){
    ...
    if(!$('#page-enquiries')){const page=document.createElement('section');page.className='page';page.id='page-enquiries';page.innerHTML=`
      ...
      <div class="enquiry-toolbar"><label class="search-box"><span>⌕</span><input id="enquirySearch" type="search" placeholder="Search name, phone, location or product"></label><select id="enquiryStatus"><option value="all">All statuses</option>${statuses.map(s=>`<option value="${s}">${label(s)}</option>`).join('')}</select></div>
  ...
  function showPage(){
    ...
    $('#page-enquiries').classList.add('active');$('.nav-item[data-page="enquiries"]').classList.add('active');$('#pageTitle').textContent='Enquiries';$('#sidebar')?.classList.remove('open');load();
  }
  function haystack(e){return `${e.customer_name} ${e.phone} ${e.business_name||''} ${e.location||''} ${e.notes||''} ${(e.enquiry_items||[]).map(i=>i.product_name).join(' ')}`.toLowerCase()}
  ...
  function renderStats(){
    const n=enquiries.filter(e=>e.status==='new').length,a=enquiries.filter(e=>['new','contacted','quoted','confirmed'].includes(e.status)).length,c=enquiries.filter(e=>e.status==='completed').length;
    [['eqNew',n],['eqActive',a],['eqCompleted',c],['eqTotal',enquiries.length],['statEnquiries',n],['enquiryNavBadge',n]].forEach(([id,v])=>{const el=$('#'+id);if(el)el.textContent=v});
    const badge=$('#enquiryNavBadge');if(badge)badge.hidden=!n;
  }
  function renderList(){
    ...
    holder.innerHTML=list.map(e=>`<button class="enquiry-row ${e.id===selectedId?'selected':''}" data-id="${e.id}"><div class="enquiry-row-top"><span class="status-pill status-${e.status}">${label(e.status)}</span><time>${date(e.created_at)}</time></div><h3>${esc(e.customer_name||'Customer')}</h3><p>${esc(e.phone)}${e.location?' · '+esc(e.location):''}</p><div><span>${(e.enquiry_items||[]).length} product${(e.enquiry_items||[]).length===1?'':'s'}</span><b>${esc((e.enquiry_items||[])[0]?.product_name||'Product enquiry')}</b></div></button>`).join('');
    holder.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>select(b.dataset.id));
  }
  ...
  function renderDetail(e){
    ...
    const ref=e.id.split('-')[0].toUpperCase(),msg=`Hello ${e.customer_name||''}, this is YK Electric regarding your enquiry YK-${ref}.`;
    d.innerHTML=`<div class="enquiry-detail-head"><div><small>YK-${ref}</small><h2>${esc(e.customer_name||'Customer')}</h2><p>Received ${date(e.created_at)}</p></div><select id="detailStatus">${statuses.map(s=>`<option value="${s}" ${s===e.status?'selected':''}>${label(s)}</option>`).join('')}</select></div>
      ...
      <div class="enquiry-products"><h3>Requested products</h3>${(e.enquiry_items||[]).map(i=>`<div><span>${esc(i.product_name)}</span><b>Qty ${i.quantity}</b></div>`).join('')}</div>
  ...
  async function updateStatus(e,status){
    const previous=e.status;e.status=status;renderStats();renderList();renderDetail(e);
    try{await rest(`enquiries?id=eq.${encodeURIComponent(e.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status,updated_at:new Date().toISOString()})})}
    catch(err){e.status=previous;renderStats();renderList();renderDetail(e);alert(err.message)}
  }
  async function load(){
    const holder=$('#enquiryList');if(holder)holder.innerHTML='<div class="enquiry-loading">Loading enquiries…</div>';
    try{enquiries=await rest('enquiries?select=id,customer_name,phone,business_name,location,notes,status,source,created_at,updated_at,enquiry_items(id,product_id,product_name,quantity)&order=created_at.desc');renderStats();renderList();if(selectedId)renderDetail(enquiries.find(e=>e.id===selectedId))}
    catch(e){if(holder)holder.innerHTML=`<div class="enquiry-loading error"><b>Could not load enquiries</b><span>${esc(e.message)}</span></div>`}
  }
